...
export class OpenAIReasoner {
  ...
  constructor(model = process.env.OPENAI_MODEL || process.env.PI_MODEL || "gpt-5.5") {
    this.model = model;
    const apiKey = process.env.OPENAI_API_KEY || process.env.PI_API_KEY;
    if (apiKey) {
      this.client = new OpenAI({
        apiKey,
        baseURL: process.env.OPENAI_BASE_URL || process.env.PI_BASE_URL || undefined
      });
    }
  }
  ...
  async refineCapability(capability: CapabilityContract): Promise<CapabilityContract> {
    ...
    try {
      response = await this.client.responses.parse({
        ...
        instructions: [
          ...
          "Classify only the observed behavior. Distinguish business CRUD/review from authenticate, upload, download, and generic action. For ambiguous POST requests, use UI text and the recorded transport semantics; do not default every POST to create.",
          "Every evidenceIds item must be copied from the provided capability.",
          "Write the title and description in concise, user-facing Chinese."
        ].join("\n"),
    ...
    return {
      ...
      sideEffect: ["create", "update", "review", "delete", "upload", "action"].includes(parsed.operation),
      confirmation: {
        required: ["create", "update", "review", "delete", "upload", "action"].includes(parsed.operation),
        reason: (["create", "update", "review", "delete", "upload", "action"].includes(parsed.operation)
          ? "该操作会改变业务或文件数据"
          : undefined)
      },
      generated: {
