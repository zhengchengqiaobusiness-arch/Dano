  ...
  async start() {
    ...
    args.push(
      ...
      [
        ...
        "snapshot.formFields/todoFields are the visible field checklist for the active page or dialog, including sibling inputs in one group, prompt placeholders such as「请输入项目名称」, nearby-label inputs, readonly choosers, empty picker slots, radios, shadow-root fields, same-origin iframe fields, and other tabs. 「请选择」and a required number 0 are empty. When the user requires every field filled except upload, call exercise-form then submit-form. Do not skip optional or blank fields and do not special-case one business page. Locate a field inside its own group; never the first input in the dialog. Choosers must pick a visible option or dialog row, not a typed sample. submit-form is not success unless a form request is seen or the form closes. If a planned write submit fails validation, repair the form and submit again; do not stop or export without a successful write response.",
