  ...
  async start() {
    ...
    args.push(
      ...
      [
        ...
        "When the user explicitly requires operations, pass every requested operation in business_skill_record_start.expectedOperations. Map 查询=query, 新增=create, 修改/编辑=update, 审核=review, 删除=delete, 上传=upload, 导出/下载=download. action is only a custom business action such as 撤回 or 签章; never map 导出 to action. Never omit the list: review and export use it to reject a partial Skill.",
        ...
        "When the user requires every accessible page, pass business_skill_record_start.completePageCoverage=true. Use snapshot.navigationCoverage and business_browser_control action=next-page to exhaust real same-origin menu links. Do not count pages from memory or invent routes; record_stop rejects every discovered but unvisited page.",
        "If completeFieldCoverage is true, the first mutation in each page/form must be exercise-form. The runtime rejects direct fill/select/choose/press before that call; use direct field actions only to repair returned todoFields after the whole-form pass.",
        "After every mutating business_browser_control call, inspect recordingAudit before choosing the next action. This is the authoritative live closed loop: it rebuilds the current capabilities, repairs field contracts deterministically, validates them, and reviews them while the browser is still open. Do only recordingAudit.nextAction, then inspect the next recordingAudit. Never postpone evidence or contract review until record_stop or export. Continue until recordingAudit.ready=true. The workbench automatically wakes the same recording if fields, operations, successful responses, pages, or request contracts are still missing.",
        ...
        "Identify the current page by snapshot.url (hash/path) plus snapshot.pageHeading or the current navigationInventory item. document.title often appends a product suffix such as 「企业管理一体化平台」 and is not a mismatch by itself. If the requested URL hash already matches, continue recording; do not stop because document.title differs from the menu name.",
      ...
      "snapshot.formFields/todoFields are the visible field checklist, including empty assignee wells in a process rail. The node title is not a filled value. When the user requires every field filled except upload, the first exercise-form call is authoritative and must fill all currently visible eligible fields in one pass; never decompose the initial form into one-field calls. A second or third call may repair only returned todoFields, newly revealed rows/fields, tree choices, format constraints, or a returned failure. Before submit-form can send create/update/review/delete/upload/custom-action data, show the current target and filled values, stop, and ask the user for explicit confirmation in a new message; the initial request to record or create is not action-time confirmation. Only after that new confirmation may you call submit-form. submit-form follows a nested application confirmation dialog, checks both HTTP and the response business envelope, and reselects an explicitly named missing chooser before automatically retrying. The first or second failed automatic operation is not a stop: re-read the current scope and repair the returned evidence; after a failed direct selector, use a newly grounded selector instead of blindly replaying the unchanged selector. Only actual failures consume the repair budget, and a successful automatic operation clears the consecutive-failure streak. On the third consecutive failed automatic operation, the current tool request pauses, Studio switches to manual mode, and a takeover card tells the person what to do. A snapshot never requests takeover by itself. Do not issue another tool call or stop/analyze while takeover is pending; wait until the person clicks 我已完成，继续自动执行. The tool then returns resumedAfterManualTakeover with a fresh snapshot; continue from it. The budget is per page/form scope. Never loop snapshot-click-snapshot or make a fourth consecutive failed repair. record_stop itself refuses to close the browser while a required operation lacks a business-success response. Locate must be unique for that field; never the first input in the dialog. Choosers click that field's shell once and pick a visible option, tree node, or dialog row, not a typed sample. Do not export a planned write without a successful write response.",
        ...
        "When the user's goal includes producing a Skill, call business_skill_export only after record_stop has accepted recordingAudit.ready=true. Capability construction, request-contract repair, validation, and review happen during recording after each action; export must not be the first place that discovers or repairs them. Export performs only a final consistency review and writes the already verified package. If recordingAudit reports a real evidence gap, keep the same browser open and handle only that gap. Never hand repairable findings or generated files to the user. Manual input is allowed only for login, the existing three-failure page takeover, an evidence ambiguity the system cannot decide, or an explicit write/composition confirmation. Never freeze a recorded sample. Report only this session's 主能力 and 字段候选接口; do not go back to another page's 新建 because the merged catalog still has it. User/product pickers are not 主能力. A recorded keyword/filter request that returns rows/list/records is this page's 查询, even if the path is getXxx and not /list or /search. A recorded conversational ask with sys_query/question/prompt is also 查询, not a chat-send action; save_*chat* and getappid are lookups. Do not ask the user to rerecord that search. Do not claim 编辑/删除 unless those writes were recorded and verified."
      ].join(" ")
    );
