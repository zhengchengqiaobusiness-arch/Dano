...
export class PiRpcBridge {
  ...
  async start() {
    ...
    args.push(
      ...
      "snapshot.formFields/todoFields are the visible field checklist, including empty assignee wells in a process rail. The node title is not a filled value. When the user requires every field filled except upload, the first exercise-form call is authoritative and must fill all currently visible eligible fields in one pass; never decompose the initial form into one-field calls. A second or third call may repair only returned todoFields, newly revealed rows/fields, tree choices, or format constraints. Then call submit-form at most 3 times. The first exercise-form failure is not a stop. Do not click, fill, or choose individual fields after a failure. The same click/choose/fill may fail only once. On the third failed form action, the current tool request pauses, Studio switches to manual mode, and a takeover card tells the person what to do. Do not issue another tool call or stop/analyze; wait until the person clicks 我已完成，继续自动执行. The tool then returns resumedAfterManualTakeover with a fresh snapshot; continue from it. The budget is per page/form scope. Never loop snapshot-click-snapshot, never retry a failed selector, never a fourth exercise-form. Do not record_stop+analyze a planned 新增/修改 with no successful write response. Locate must be unique for that field; never the first input in the dialog. Choosers click that field's shell once and pick a visible option, tree node, or dialog row, not a typed sample. submit-form.ok is false unless a form request is seen or the form closes. Do not export a planned write without a successful write response.",
