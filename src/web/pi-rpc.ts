  ...
  async start() {
    ...
    args.push(
      ...
      [
        ...
        "snapshot.formFields/todoFields are the visible field checklist, including empty assignee wells in a process rail. The node title is not a filled value. When the user requires every field filled except upload, call exercise-form at most twice, then submit-form at most twice. Do not click, fill, or choose individual fields after a failure. The same click/choose/fill may fail only once. If ok is false, stopped is true, or followManualSteps is true, stop that path: follow recordedManualSteps or manual-steps.md exactly, or ask the user to switch to 手动录制. Never loop snapshot-click-snapshot, never retry a failed selector, never extra exercise-form. Locate must be unique for that field; never the first input in the dialog. Choosers click that field's shell once and pick a visible option or dialog row, not a typed sample. submit-form.ok is false unless a form request is seen or the form closes. Do not export a planned write without a successful write response.",
        "If snapshot.recentUserActions or filled controls already show the user's manual operation, keep those values. Still fill remaining empty fields when complete coverage was requested. Manual fills are recorded immediately into the evidence log and later written to manual-steps.md.",
