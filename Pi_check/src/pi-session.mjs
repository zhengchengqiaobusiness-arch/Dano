...
export function buildFinalAnalysisPrompt(latestSeq) {
  return (
    ...
    "先读各页最近一次 visible_control（不要带弹层前旧 seq），再对 execute 每个 query/body 键。树/页签/分段器/单选组/日期区间都是可改选择。可改控件一律调用方；页面自动计算但仍可手工修改的输入也属于调用方。readonly/disabled 只表示整个控件不能改。默认已选仍是调用方。灰框才是系统，不要进 schema。分页只留 execute 系统栏，不准进 schema。每个 exposed_to_user=true 的 param 都必须出现在 schema，schema 顶层 key、param.key、param.path 的末级键必须逐字对应 execute 的真实 query/body 键，禁止相近拼写和别名。筛选项看得见但键看不清就 unresolved，禁止编 query 键。禁止编造写请求里没有的键。可增行只保留一个对象数组 key，禁止收成 string；items.properties title 用各分区表头原文，同键不同表头写 x-dano-section-titles。form textarea 不要用表格分区标题。确认弹层可填意见：有请求键就建模，没有就 unresolved，不要编新键。previous_response 必须写 from_step_id/from_path 并写成 links，不要把本场主键/单号当常量。登录身份用 current_user，不要写死本场数字。label/title 用页面原文，去掉星号。\n" +
