...
- **可增行明细**：调用方输入的是点「添加×× / 新增行」后单独出现的那些输入框，行数可变。`input_schema` 只出现 execute 里那一个数组 key。`items.properties` 的 `title` 用**表头原文**（不要自造合并名去替代列名）；数组自己的 `title` 用各分区标题原文，多个分区分号或 `/` 连接。`reason` 必须写清：调用方按行填写这些实际输入，可增减；**最后**由系统把各行组装成 execute 里的那一个对象数组，并按录制原值补上无独立控件的行字段。禁止把整份数组当成调用方一次性粘贴的 JSON。禁止把一个数组拆成多个调用方数组或并列字段去抢同一 path。
...
29. 可增行 `items.properties` 的 title 是表头原文；数组 title 是分区标题原文。
...
- 换一个页面也走同一套台账 / 切分 / 编排 / 字段形状规则：先摊请求形状，再摊可见控件，对得上可改控件就是调用方且必须进 schema；`readonly`/`disabled` 灰框是系统；树/页签/区间日期同样是调用方；可增行按行填写再组装成一个数组 path，列名用表头原文，不要把同名 textarea 当成行，也不要把一个数组拆成多个调用方数组；空容器有上传/选人/可增行控件仍是调用方；确认弹层可填意见完整处理，没有请求键就 unresolved，不要编造写请求里没有的键；无控件的 query/body 键按身份或原值交给系统，并写进系统栏。名字与页面原文一致，去掉星号。
