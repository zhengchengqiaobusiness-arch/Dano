...
你是在按目标把业务动作做完，并留下那次真实操作的合同。不是尽快点出一条请求。

### 目标是剧本

按目标原文的顺序，在每一页把该页的事做完，再离开这一页。
...
以后再碰到识别缺口，先改本文件。不要给某一页、某个 URL 或某个字段打补丁，也不要改采集/导出代码去猜业务。

- 代码（稳定层）：启动浏览器、原样采集证据、冻结、把你的 result 原样交给前端和导出。导出只运输你写进信封的合同：调用方字段、`x-dano-section-titles`、行系统字段、`links`、`capability_relations`。可提供 `list_recording_index` 这种无分类事实索引。只拒收页面读不到的信封、重复的 `capability_id`、两个能力共用同一个 `execute`。不识别页面，不补能力，不从目标原文猜「先查后增」，不把你标成 `constant` 的值改写成 `now`。
- Skill（本文件）：跨页面通用的总入口与识别方法。改调查顺序、字段归属、分区写法、有无值流的组合路线，都改本 Skill。
- 模型（你）：阅读本场证据，判断“这是几个独立业务动作”、每个字段谁提供、分区怎么切、有没有值要跨能力传递、目标是否要求先 A 后 B。不同页面由你现场判断，不要套死某个系统的 URL。
...
4. **按目标写上再证明绑定**：先按目标把该动作的可见字段写上并点出 execute，再对还没对上的键改一个值看哪个请求键变了。禁止用空表保存换请求形状。值碰巧相等、字段名相似、同一个请求里仅剩两个字段，都不能单独定案。本场没改过、但控件可改：仍是调用方，不要写成 `constant` / 「无独立来源」。无法做这个实验（合法 selector 写不进、未授权写入）：该键写入 `unresolved`，或按下面「无独立来源，按录制请求原值提交」处理；禁止编 path，禁止 invent selector。来源没看清不能说成已经理解业务。`default_value` 只固定无来源字段怎么提交，不是下次执行必须填的业务值。
5. **逐键对上控件，决定调用方还是系统**：
   ...
   - 请求里有、控件上没有：行类型判别、行序号、前端行键、前端时间戳 → **系统**，`source_kind=constant`，`default_value` = 本场请求原值。reason 写「无独立来源，按录制请求原值提交」。
...
- execute 请求体/查询里的每个业务字段都要出现在该 step 的 `params`，不要只写人改过的那几个。没有独立来源的键也要留下，标系统自动处理，按请求原值传递，不做任何改动。
...
26. execute 的每个 query/body 键都在该 step 的 `params` 里。没有独立来源的键标系统自动处理，`default_value` 用请求原值，`reason` 写「无独立来源，按录制请求原值提交」，没有漏出系统栏。
...
35. 未接到用户结束没有调用 `submit_recording_result`。
36. 首屏自动加载没有被当成已经查询。目标要求先查询后新增时，查询条件设齐并看到该查询 execute 之前没有进入新增。
37. 没有在表单仍空、或目标要求的加行还没写上时点保存。
38. 选项接口只挂 `option_source`，没有把选项列表路径写成值流 `links`。
39. 同一张表「保存/存草稿」与「提交/送审」若 path 或效果不同，已拆成两项能力，不要并进提交。
40. 树单击是单值；schema type 与 param type 一致。多分区数组写了 `x-dano-section-titles`，合并行时带分区标题。
41. 目标写了先 A 后 B：有值流则 `links` 已写；没有值流则 `capability_relations` 已写。没有把顺序留给导出去猜。

## 泛化

- 本 Skill 不绑定任何具体业务页、系统名或字段名。上面的例子只说明形状，不是某页的补丁。
- 只根据本场点击、输入、请求、响应、`visible_control`、截图判断。
- 换一个页面也走同一套台账 / 切分 / 编排 / 字段形状规则：先按目标把该页动作做完再离开；首屏自动请求不是已经查询；空表不要点保存；先摊那次真实操作的请求形状，再摊可见控件，表上已经按目标填过后再改一个值看哪个请求键变了；值相等、字段名相似、排除法不能单独定案；对得上可改控件就是调用方且必须进 schema；`readonly`/`disabled` 灰框是系统；树/页签/区间日期同样是调用方；树单击是单值；弹层选人/选记录若提交对象数组，就保留一个多选字段并用 `element_template` 从实时选项行组装对象；可增行按行填写再组装成一个对象数组 path，列名用各分区表头原文，数组 title 用 `分区A/分区B`，同一键不同表头写 `x-dano-section-titles`，合并行时带分区标题，不要把同名 textarea 当成行，也不要把一个数组拆成多个调用方数组或收成一段字符串；选项接口只挂 `option_source`，禁止把选项列表路径写成值流 `links`；目标先 A 后 B 且没有值流时写 `capability_relations`；同一张表保存与提交若 path 或效果不同必须两项能力；空容器有上传/选人/可增行控件仍是调用方；确认弹层可填意见完整处理，没有请求键就 unresolved，不要编造写请求里没有的键；无控件的 execute query/body 键按身份或原值交给系统，并只写进 execute 系统栏；打开表单附带列表与弹层翻页不进写入能力系统栏。名字与页面原文一致，去掉星号。
...
以后再碰到识别缺口，先回到本文件补**通用形状规则**，不要改采集/导出代码，也不要给某一页打补丁：

- 首屏自动请求不是已经查询；空表保存不是调查。
- 点加行才出现的输入框 → 调用方按行提供，系统再组装成一个数组。
- 多分区可增行 → 数组 title 用 `分区A/分区B`，表头不同就写 `x-dano-section-titles`；不要写成拆不开的「A和B」。
- 弹层勾选记录、execute 提交对象数组 → 调用方选择显示项，系统按实时响应组装原数组对象，不能写死本场所选行。
- 同名大段 textarea ≠ 这些行。
- 确认弹层可填意见必须完整处理：对不上请求键就 unresolved，不要编 path。
- 分页是系统，不准进 schema。
- 跨能力有值流 → `links`（from/to step + path）。目标先 A 后 B 且没有值流 → `capability_relations`。不要拿本场样例当合同，也不要让导出猜顺序。
- 请求有、控件无、也看不出公式 → 系统栏 + 原值传递。你标 `constant` 的值，导出按原值提交，不会改成 now。
