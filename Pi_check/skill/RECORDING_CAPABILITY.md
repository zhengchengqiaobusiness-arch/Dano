...
8. **没打开过的下拉**：不要编 `enum_options`，也不要用列表单元格或另一页的值冒充选项。写入 `unresolved`，标明缺的是当场下拉选项。打开过的必须列当场全部选项，并标 `options_complete=true`。
9. **同一 path 只能有一种归属**。禁止同一个 `query.xxx` / `body.xxx` 既写成调用方又写成系统。
...
15. 同一 path 只有一种归属。系统主键没有进 `input_schema`。
...
- 证据不够、台账对不齐、字段来源写不出时：写入 `unresolved`，不要猜测成看似可用的残缺能力。导出层看到 `unresolved` 或来源未确认的写入字段会直接拒绝，不会补全。
