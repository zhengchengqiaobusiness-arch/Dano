...
6. **定稿能力** → 目标原文要求的行都齐、每一行合同完整、写入没有「未识别却当可执行」、且没有同名同 execute 的重复行时，立刻 `submit_recording_result({final:true, use_draft:true})`。目标写了查看/详情，台账里还没有该行的 execute、也没有对应 `unresolved`：禁止定稿。目标没要求的行（路过的导出、列表首屏加载）即使合同完整也不要定进结果。草稿里已有重复或多余项：禁止 `use_draft`；`submit_recording_result` 的 `result` 只带目标要求的那几项。不要等用户说结束。不要调 Skill 4。
