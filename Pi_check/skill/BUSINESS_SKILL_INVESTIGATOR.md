...
- **查看 / 详情**：目标写了点结果里的数字、链接或单元格看详情，这是台账里单独一行，不是查询的附带动作。列表查询发出、筛选项设齐，都不算详情做过。必须再点 snapshot 里 `region=table` 的该入口，弹出层或跳到明细后，发出**这条详情自己的**请求，这一行才算做完。点到导出/下载、同名树节点或另一行姓名，都不算，也不要用已经发出的导出/列表请求顶替这一行。做不成就写入 `unresolved` 说明缺哪条详情请求，禁止默默丢掉再换页、禁止把整场压成 query+create 定稿。
...
2. **按目标操作** → Skill 2：把该动作需要的可见条件/字段写上，再点该动作自己的查询/保存/提交/详情。没有「搜索 / 查询」文案时，点已经出现的树或列表节点就是查询，不要为找搜索钮去点没文案的 `aN`。目标要看详情：列表查询发出后仍留在这一页，再点结果表里该详情入口；不要把列表查询当成详情已做完。只点当前业务区。通知角标、头像、偏好、退出、外观不是业务。空状态提示是还没做成，不是按钮。灰框不要硬点。
...
6. **定稿能力** → 目标原文要求的行都齐、每一行合同完整、写入没有「未识别却当可执行」、且没有同名同 execute 的重复行时，立刻 `submit_recording_result({final:true, use_draft:true})`。目标写了查看/详情，台账里还没有该行的 execute、也没有对应 `unresolved`：禁止定稿。目标同时要「查缺项/详情」和「新增」：详情行未做完就禁止把整场压成查询→新增定稿。目标没要求的行（路过的导出、列表首屏加载）即使合同完整也不要定进结果。草稿里已有重复或多余项：禁止 `use_draft`；`submit_recording_result` 的 `result` 只带目标要求的那几项。不要等用户说结束。不要调 Skill 4。
...
4. 页内顺序已交给 Infer（有值流的 `links`，或没有值流的 `capability_relations`）。本场已做完的查询和详情必须挂「查询→详情」，禁止只留下查询→新增。没有顺序时按能力在合同中的次序记账，仍可定稿能力；默认办理路线由之后的 Skill 4 出包会话编写。
...
- 缺依据 / 未识别当已解决 / 灰框进了调用方 / 假 links / 错挂 preflight / 可改控件收成系统 / 系统栏填不出 / option_source 挂成另一能力 execute / 默认关系跳过详情 → Skill 3
...
→ 按目标写字段；每种加行都点并写上；再点该动作自己的查询/保存/提交/详情；详情未发出自己的请求就不要离开这一页
→ 阻断才 assist；解除后自动继续
