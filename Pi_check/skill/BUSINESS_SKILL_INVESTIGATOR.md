...
- **查看 / 详情**：目标写了点结果里的数字、链接或单元格看详情，这是台账里单独一行，不是查询的附带动作。列表查询发出、筛选项设齐，都不算详情做过。必须再点该入口，弹出层或跳到明细后，发出**这条详情自己的**请求，这一行才算做完。点到导出/下载、同名树节点或另一行姓名，都不算。做不成就写入 `unresolved` 说明缺哪条详情请求，禁止默默丢掉再换页、禁止把整场压成 query+create 定稿。
...
6. **定稿能力** → 目标页与台账该交的行都齐、每一行合同完整、写入没有「未识别却当可执行」、且没有同名同 execute 的重复行时，立刻 `submit_recording_result({final:true, use_draft:true})`。目标写了查看/详情，台账里还没有该行的 execute、也没有对应 `unresolved`：禁止定稿。草稿里已有重复：禁止 `use_draft`；`submit_recording_result` 的 `result` 只带正确那几项。不要等用户说结束。不要调 Skill 4。
