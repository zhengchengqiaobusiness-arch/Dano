...
5. 选项、分段、单选、页签、已经出现的树/列表节点用 `choose(selector, 可见原文)` 或 `text=`。不要 click 后再猜新 selector。筛选框只过滤；列表/树变了还要选已经出现的可见节点。**没有「搜索 / 查询」文案时，点已经出现的树或列表节点就是查询。** 不要为找搜索钮去点没文案的 `aN`。打开弹层、切换页签或加行后再 snapshot **一次**。加行后看不到行内框：协助这一格，不要点保存。
...
- 协助发出之后：禁止再 click。只读 `recentUserActions` 和 `network_since`。直到人做完或用户说继续，不要自己再点。人已经发出预期请求就停手，交给识别 Skill 交能力。不要整张表重做。
...
只在这些情况用 `action=assist`：登录、验证码、授权写入、**写不进的那一个字段**、**点了不发网的那一个提交钮**、**加行后看不到的行内框**。`reason` 写清楚要人做什么。预览始终可以点。不要把协助做成 takeover，不要让程序丢掉人的 `applyInput`。禁止一次 assist 把整张表交出去然后自己空转。协助后不要自己再点那一个钮。
