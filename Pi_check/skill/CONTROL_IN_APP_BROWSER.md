...
禁止：`name=`、`#id`、`.class`、xpath、任意 CSS，以及 snapshot 里没有的字符串。不要把失败后的猜测写成新 selector。没有业务文案的 `aN`、顶栏未读/消息纯数字角标、头像不要点。snapshot 若仍列出它们，忽略。结果表里可点的数字/文案链不是角标：它们会出现在 snapshot `actions` 里，`region=table`。用那一条的 `ref=`，不要用光秃数字 `text=`。
...
值由 Investigator 指定。工具不自填样例，不整表自动填充。多个普通输入框：一次 `fill_fields`，**必须带非空 `fields[]`**（每项 `ref`/`selector` + `text`）。空 `fill_fields` 会失败，不要拿来探路。每项只写该项自己的 `ref`，禁止把标题写进日期框、把日期写进标题框。未打开目标表单时不要 fill。

选项、分段、单选、页签、已经出现的树/列表节点用 `choose(selector, 可见原文)` 或 `text=`。不要 click 后再猜新 selector。筛选框只过滤；列表/树变了还要选已经出现的可见节点。

结果表里看详情：先 snapshot，在 `actions` 里找 `region=table` 且带列头/行区分的那一条，用它的 `ref=`。不要用光秃数字 `text=`（会打到左侧树、汇总卡或其它行）。不要点「下载 / 导出」代替详情。格子不在 snapshot `actions` 里、或看起来只是纯文本：不要猜 `text=`，不要去读页面脚本；换同一列已经广告出来的可点格，或只协助这一格。点开后立刻 snapshot **当前弹层**，用 `network_since` 对上这条详情自己的请求；对上导出文件就停，回到结果表再点该详情格。Investigator 点名多种列：回到结果表，对**每一种列**各点一次广告出来的格子，不要只点第一种就取消弹层当全部做完。点完用 `network_since` 把各次详情 URL 交给 Infer：path+query 相同就是同一 execute，不要假装点出了多项能力。
...
加行后用新列表里 `region=table` 的 `label=表头` / `placeholder=` 写每一列；不要把整张表当成一个控件。每一种「添加××」分区都要单独加一次、写一次，每加一种分区再 snapshot 一次，好让表头和分区标题进证据。某一列只在一种分区表头出现（例如只有工作行有进度），加的就是那一分区，不要用另一分区的空表头去宣称该列不存在。只有这一列在新 snapshot 里仍然没有可写控件时，才协助这一格。
...
Investigator 要求证明某日期或下拉可改时：必须真正换一个值（换一天、换一个可见选项），`network_since` 看到对应请求键变化。不要因为框里已经有当天或默认选项就跳过。内部展示框带原生 `readonly` 仍要点开面板改，不要当灰框。
...
只认 **当前 URL 这一页** 的 `visible_control`。snapshot 里若混进上一张表的控件和新页筛选条，只保留当前 URL 对得上的那一组。禁止把无关路由上的控件交给 Infer 去认刚做完的写能力。

当前页目标已经做完：**停止自动点**。不要点侧栏其它业务模块，不要 `open_page` 到目标原文没有的路由。「离开这一页」不是换一个菜单。Investigator 点名补证除外。

写请求已经发出且业务成功：立刻停止 click / fill / choose / open_page。若 SPA **没有任何 click** 就改到目标外路由：回报 Investigator，不要跟着点，不要用新页 snapshot 当这张表的字段证据。

附件 multipart 已经发出：不要再点「预览 / 下载 / 附件」做验证。提交钮才是写能力收口。目标原文没有要求预览附件时，预览不是业务动作。
...
页面与请求对不上时，用 `read_page_asset({url})`。仅本场已加载的同源 URL。禁止读用户其他项目代码。公开前端是辅助证据，必须与实际页面或请求交叉核对。结果表缺可点格时不要靠读脚本猜 selector，只协助这一格，禁止当查询完成离开。
...
人已经离开当前表单（回到列表、关掉弹层、打开另一页）：禁止再点刚才那张表的保存/确认。当前 snapshot 的 `region` 已经不是那张表，就不要再点。当前页目标还没做完：禁止 `open_page` 去下一页。写请求已业务成功：停止点，即使当前落到 404 或非该入口页也不要自己 `open_page` 乱跳；回报 Investigator。登录需要用户处理时保留当前会话。
...
- 自动点击失败、空转或超时：只停自动点，不要结束录制，不要丢掉人的 `applyInput`。空转是 `transport_idle`，不是业务失败。目标行已做完时不要靠空转去「再看一眼」别的模块。超时后不要再 click，包括不要点附件预览。
- 不要等人全部点完再读几百条证据拼超大 JSON。
- 不要本地推断能力，不要交能力或定稿。
- 不要改 Vue、React 或其他框架的内部业务状态来「修好页面」。
- 正常交互形不成有效请求：报告实际问题，不能制造修补后的请求再当作自然页面行为。
- 批量填写只接受指定输入，不自行决定所有业务样例。
- 禁止点侧栏/顶栏里目标原文没有的业务菜单。
