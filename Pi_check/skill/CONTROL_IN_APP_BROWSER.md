...
结果表里看详情：先 snapshot，在 `actions` 里找 `region=table` 且带列头/行区分的那一条，用它的 `ref=`。不要用光秃数字 `text=`（会打到左侧树、汇总卡或其它行）。不要点「下载 / 导出」代替详情。格子不在 snapshot `actions` 里、或看起来只是纯文本：不要猜 `text=`，不要去读页面脚本；换同一列已经广告出来的可点格，或只协助这一格。点开后立刻 snapshot **当前弹层**，用 `network_since` 对上这条详情自己的请求；对上导出文件就停，回到结果表再点该详情格。Investigator 点名多种列：回到结果表，对**每一种列**各点一次广告出来的格子，不要只点第一种就取消弹层当全部做完。
