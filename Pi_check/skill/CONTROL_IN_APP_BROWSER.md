...
只在这些情况用 `action=assist`：登录、验证码、授权写入、**写不进的那一个字段**、**点了不发网的那一个提交钮**。`reason` 写清楚要人做什么。预览始终可以点。不要把协助做成 takeover，不要让程序丢掉人的 `applyInput`。禁止一次 assist 把整张表交出去然后自己空转。协助后不要自己再点那一个钮。
