...
test("page viewport stays desktop sized while preserving the preview pane aspect ratio", () => {
  ...
  assert.deepEqual(normalizePreviewViewport({ width: 500, height: 400 }), { width: 1440, height: 1152, scale: 1 });
  assert.deepEqual(normalizePreviewViewport({ width: 1100, height: 700 }), { width: 1440, height: 916, scale: 1 });
  assert.deepEqual(normalizePreviewViewport({ width: 1800, height: 720, scale: 1.5 }), { width: 2250, height: 900, scale: 1 });
...
test("recording page uses a desktop viewport instead of the preview pane", async () => {
  ...
  assert.match(app, /async function rememberPaneViewport\(/);
