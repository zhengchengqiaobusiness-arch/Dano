...
test("exported executor maps colliding detail question ids without overwriting header fields", async () => {
  ...
  try {
    ...
    const prepared = JSON.parse(stdout).prepared;
    assert.deepEqual(prepared, { billType: "reimburse", items: [{ billType: "0", billCount: "2" }] });
  } finally {
    await rm(temporary, { recursive: true, force: true });
  }
});
