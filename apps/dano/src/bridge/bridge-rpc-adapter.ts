 ...
 */
export interface BridgeRpcAdapterContext {
...
class SessionRuntime {
  ...
  constructor(
    private readonly context: BridgeRpcAdapterContext,
...
class ExtensionUIBridge {
  ...
  handleResponse(response: RpcExtensionUIResponse): void {
    ...
    if (!pending) {
      console.warn(
        `BridgeRpcAdapter[${this.clientId}]: Received UI response for unknown request: ${response.id}`,
      );
      return;
    }
    ...
    console.log(
      `BridgeRpcAdapter[${this.clientId}]: UI request ${response.id} (${pending.method}) resolved`,
    );
    ...
    const createDialogPromise = <T>(
      ...
      return new Promise((resolve, reject) => {
        ...
        timeoutId = setTimeout(() => {
          console.log(
            `BridgeRpcAdapter[${this.clientId}]: UI request ${id} (${request.method}) timed out`,
          );
          cleanup();
  ...
  dispose(): void {
    for (const [id, pending] of this.pendingRequests) {
      ...
      console.log(
        `BridgeRpcAdapter[${this.clientId}]: Resolving UI request ${id} (${pending.method}) on disconnect`,
      );
      pending.resolve({
  ...
  private sendRequest(request: RpcExtensionUIRequest): void {
    console.log(
      `BridgeRpcAdapter[${this.clientId}]: Sending UI request ${request.id} (${request.method})`,
    );
    this.send({
      type: "extension_ui_request",
      payload: request,
...
export class BridgeRpcAdapter {
  ...
  constructor(
    client: BridgeClient,
    send: RpcAdapterSend,
    context: BridgeRpcAdapterContext,
    config: BridgeConfig,
  ...
  private sendResponse(message: ServerMessage): void {
    ...
    try {
      ...
      console.error(
        `BridgeRpcAdapter[${this.client.id}]: Failed to send message:`,
        err,
      );
    }
  }
  ...
  dispose(): void {
    ...
    console.log(`BridgeRpcAdapter[${this.client.id}]: Disposing adapter`);
