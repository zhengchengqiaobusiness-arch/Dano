...
const MARKDOWN_FIXTURE_LINES = [
  ...
  "- Bold: **important**",
  "- Italic: *subtle*",
  "- Inline code: `apps/dano/web/src/components/ChatTranscript.svelte:564`",
  "",
  "| Column | Value |",
  "| --- | --- |",
  "| Mode | Debug |",
...
const BASH_FIXTURE_ARGS = {
  command: [
    "pnpm -C apps/dano check:web",
    "pnpm run build:web",
    'rg -n "MarkdownRenderer" apps/dano/web/src/components/ChatTranscript.svelte',
  ].join("\n"),
  timeout: 180,
};
...
const BASH_FIXTURE_FINAL_OUTPUT = [
  ...
  "rendering chunks...",
  "computing gzip size...",
  "dist/web/assets/index.js 536.07 kB | gzip: 184.87 kB",
  "dist/web/assets/vendor-mermaid.js 1557.84 kB | gzip: 489.70 kB",
  "apps/dano/web/src/components/ChatTranscript.svelte:564:                <MarkdownRenderer",
  "Done in 1.03s.",
...
const EDIT_FIXTURE_ONE_DIFF = [
  "--- a/apps/dano/web/src/App.svelte",
  "+++ b/apps/dano/web/src/App.svelte",
  "@@ -78,7 +78,10 @@",
  "-const debugWorkspaceSummary = createDebugWorkspaceSummary();",
  "+const debugWorkspaceSummary = createDebugWorkspaceSummary({",
  '+  accent: "violet",',
...
const EDIT_FIXTURE_TWO_ARGS = {
  path: "apps/dano/web/src/components/MarkdownRenderer.svelte",
  edits: [
    {
      oldText:
...
const EDIT_FIXTURE_TWO_DIFF = [
  "--- a/apps/dano/web/src/components/MarkdownRenderer.svelte",
  "+++ b/apps/dano/web/src/components/MarkdownRenderer.svelte",
  "@@ -193,7 +193,11 @@",
  "-function replaceWithMermaidSource(block: HTMLElement, source: string, statusText: string) {",
  "+function replaceWithMermaidSource(",
  "+  block: HTMLElement,",
...
const WRITE_FIXTURE_PATH = "apps/dano/web/src/utils/debugPreviewRegistry.ts";
...
function progressiveReadFixture(session: DebugSession): DebugPromptResult {
  ...
  const outro = [
    ...
    "- File references like `apps/dano/web/src/components/ChatTranscript.svelte:564` should stay clickable.",
  ].join("\n");
...
function progressiveMixedFixture(session: DebugSession): DebugPromptResult {
  ...
  const finalMarkdown = [
    ...
    "- Inline file refs like `apps/dano/web/src/components/MarkdownRenderer.svelte:243` should stay clickable.",
  ].join("\n");
