...
### 4.7 录制语义上下文与缓存 — `flow_spec._SemanticConversation`

- 首次 `plan` 必须返回 `dano.capability-semantic-plan.v1` 语义蓝图，覆盖全部已物化步骤、全部对外字段、能力和能力关系；不是只修 Validator finding。
- 一个 PI run 内复用完全相同的 `system + 稳定录制事实` 消息前缀，Planner 结果和 Validator 差量按消息追加；不再把每轮完整 FlowSpec 当成无历史的新请求。
- 后续优化携带上次已接受 `semantic_plan` 的压缩语义记忆，但 `scope_locked=true`，不能改变能力身份、类型和接口成员；只输出 `reviewed_scope + ops + unresolved_items`，不重述完整蓝图。
- 模型 Proposal 只在副本中应用；出现新错误、Wire 变化、dry-run 退化或增量范围变化时整轮拒绝。
- 完全相同的已优化工作台用 `last_optimization_input_hash` 直接短路，不再调用模型。真实 `prompt/cached/completion tokens` 由 `OpenAICompatClient` 记录。
- `page_required` 是页面证据，`caller_required` 是调用合同；登录态、常量、上游响应、系统时间、计算值和页面上下文来源即使页面必填，也不得进入调用方 `required`。
