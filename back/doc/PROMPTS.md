...
| `execution/page/flow_spec.py:_FLOW_SEMANTIC_SYSTEM` | 录制首次完整语义蓝图 + 后续补强 | ✅ 对象 | 稳定事实白名单、完整覆盖检查、受限 FlowEdits、候选副本 Validator/dry-run 单调准入 |
...
### 4.7 录制语义上下文 — Pi `AgentSession`

- 每个 opaque `recording_id` 只对应一个持久化 Pi `AgentSession`；Node 运行时使用 `SessionManager.create/open` 创建或恢复会话，Python 和前端都不重建消息历史。
- 录制模式只开放五个受控工具：`get_recording_state`、`submit_recording_plan`、`get_validation_report`、`submit_recording_repair`、`submit_recording_review`。事实读取、规划、修复和审核均通过这些工具进入后端权威状态。
- `plan`、`repair`、步骤命名、业务说明和发布审核复用同一会话。每次任务先读取最新后端状态或验证报告，再提交带 `base_flow_version` 的结构化候选；会话记忆不能替代当前事实和版本检查。
- 消息历史、上下文压缩和失败重试由 Pi 原生 `AgentSession`、`SessionManager`、`SettingsManager` 管理。录制应用层不实现模型响应缓存、手工消息数组、摘要记忆、重试循环或压缩算法。
- 后端只负责确定性 Schema、事实、版本、权限和发布闸门校验；工具拒绝时直接失败，不回退到 Python 模型直连或另一套评审链路。
- 字段只保留一个 `required` 结论；登录态、常量、上游响应、系统时间、计算值和页面上下文来源不得进入公开输入合同的 `required`。
...
| **机器强制**（确定性、普适硬错） | 代码校验 | 端点/字段白名单(`validate_plan`)、表达式合法性(`_expr_problem`) |
| **留给提示词 grounding**（因系统而异，硬判会误驳） | 模型 + 证据 | **标识是否拼进 URL 路径**——RESTful 系统 `/users/{id}` 是对的，故**不做静态硬判**,只在提示词里据"请求体示例"引导 |

> 第三类是关键设计:任何"换个企业/系统就可能不成立"的规则**都不能下沉成硬闸门**,否则会误驳正确能力方案。
...
4. **改评审 prompt？** 不要加全局"有疑即拒"（会顶翻反误判豁免);窄域 fail-closed + 保留"设计如此"豁免。
5. **新接一个 LLM 调用点？** 用 `partial(openai_text_spawn, tag=..., json_mode=True)`；输出数组就包成对象；抽取用 `prompt_utils`；不可信外部输入用 `wrap_data`。
