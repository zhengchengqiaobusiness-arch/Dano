...
def _input_forms_bundle(plans: list[dict]) -> tuple[str, dict[str, str]]:
    ...
    header = [
        ...
        "- 复制某能力的表单模板前，先删除已由当前对话提供且通过校验的字段；只询问当前步骤仍缺少的字段，不重复问有效答案。不要改 `questions[]` 的 id 或 options，也不要另编字段或候选项。",
...
def _caller_display_rules() -> list[str]:
    return [
        ...
        "- 表单以 `references/INPUT_FORMS.md` 里该能力的 `questions[]` 为准。字段和 options 必须与能力 `input_schema` 完全一致；枚举只显示契约 options 的 label，纠错仍用同一组 options。禁止增加、删除、改写 options。可选字段未选则不传该字段。",
