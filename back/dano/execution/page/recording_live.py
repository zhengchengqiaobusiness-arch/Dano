...
def merge_live_agent_state(live_spec, finalized_spec):  # noqa: ANN001, ANN202
    ...
    if isinstance(live_semantic_plan, dict):
        ...
        for item in live_semantic_plan.get("capabilities") or []:
            ...
            item["anchor_step_id"] = _resolve_live_plan_step_id(
                request_id,
            ...
            for request_ref in item.get("request_refs") or []:
                ...
                request_ref["step_id"] = _resolve_live_plan_step_id(
                    ref_request_id,
                    step_ids=merged_step_ids,
                    step_id_by_request_id=merged_step_id_by_request_id,
                    steps=merged.steps,
    ...
    if isinstance(live_semantic_plan, dict) and live_semantic_plan.get("capabilities"):
        ...
        for capability in materialized_plan.get("capabilities") or []:
            ...
            resolved_anchor = _resolve_live_plan_step_id(
                anchor,
                step_ids=step_ids,
            ...
            for request_ref in capability.get("request_refs") or []:
                if not isinstance(request_ref, dict):
                    continue
                identifier = str(request_ref.get("step_id") or "")
                resolved = _resolve_live_plan_step_id(
                    identifier,
                    step_ids=step_ids,
                    step_id_by_request_id=step_id_by_request_id,
                    steps=merged.steps,
        ...
        if compile_items:
            ...
            if compilation.errors:
                unresolved.append({
                    "op": "compile_capabilities",
                    "status": "rejected" if not compilation.capabilities else "partial",
                    "requested_target": {},
                    "reason": "; ".join(compilation.errors),
                })
        elif isinstance(live_capability_model, dict) and live_capability_model.get("semantic_plan"):
            merged.meta = {
                **(merged.meta or {}),
                "capability_model": deepcopy(live_capability_model),
            }
