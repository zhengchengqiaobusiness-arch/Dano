...
def _apply_create_form_runtime_origins(step: FlowStep) -> None:
    """Do not guess now/current_user from leaf names. Only an explicit contract may mark them."""
    del step
    return


def _repair_uncontrolled_write_state_fields(spec: FlowSpec) -> int:
    """Do not freeze unrecognized write-state leaves as recorded constants."""
    del spec
    return 0
