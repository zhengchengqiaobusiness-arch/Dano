...
# ─────────────────────────── LLM 解析(非结构化文档 → 动作清单) ───────────────────────────
_DOC_PROMPT = """你是接口文档解析器。从下面的接口文档里**抽取**所有 HTTP 接口,输出 JSON 数组。
...
- 输出**纯 JSON 数组**,不要 markdown 代码块、不要任何解释文字。
...
async def _llm_doc_to_actions(text: str, spawn) -> list[dict]:  # noqa: ANN001
    ...
    raw = await spawn(_DOC_PROMPT + text)
...
def _extract_json_array(s: str) -> list:
    """从模型输出里抠出 JSON 数组(容忍 ```json 围栏、前后噪声)。失败返回 []。"""
    if not s:
        return []
