...
class RecordingPiSession:
    ...
    async def notify_live_batch(self, delta: dict) -> dict[str, Any]:
        ...
        return await self.prompt(
            ...
            "set_param_required、set_param_enum、rename_field、propose_dependency、add_pitfall 等必要增量；"
            "步骤尚未物化时字段操作可把 request_id 填入 step_id；依赖只能先提案，禁止标 verified。"
            "必须调用 submit_recording_plan，并把上述操作放入 plan.ops；实时阶段不需要读取验证报告。"
            "参数来源六分类 user_input/constant/session_header/page_context/chained/computed："
            "user_input 必须有 fill/select 等可编辑控件证据；录制值固定的业务常量（body 里的单据类型、"
            "流程 key 等）归 constant；session_header 仅限 headers.* 路径；未被操作人修改的"
            " pageNo/pageSize/current/limit/offset 等分页值归 page_context（录制默认值且调用方可覆盖）；上游响应强值复用归 chained"
