...
from dano.shared.std_fields import is_flow_internal, standard_fields_for
...
def _bind_field(param: str, *, tenant: str = "", required: bool = True) -> FieldBinding | None:
    """把接口入参绑定到平台标准字段。
...
def _build_assertions(*, risk: RiskLevel, success_rule: str | None) -> Assertions:
    ...
    ① success_rule —— 系统约定(dialect.success_rule() 或 LLM 探得的成功约定,如 code==200);
...
def build_connector_body(action: ActionSpec, *, tenant: str, subsystem: str,
    ...
    bindings = [b for p in action.params_in
                if (b := _bind_field(p, tenant=tenant, required=p in required_set)) is not None]
