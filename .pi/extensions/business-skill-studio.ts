...
export default function businessSkillStudio(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description: "Control the active embedded browser with goto/snapshot/click/fill/select/choose/press/wait/screenshot/exercise-form/submit-form. A detected login page stops before any automatic action with loginRequired and waits for the person to complete login through the Studio manual-takeover card. Snapshot includes operationInventory and enabled availableOperations across the page and visible iframes, so complete every requested page operation rather than only opening its route. Actual clicked operations are added to the recording completion contract. The first exercise-form call is authoritative and fills every currently visible eligible field in one pass; later calls may handle only returned todoFields or fields newly revealed by that pass. Use choose for dropdowns and tree/cascader nodes. exercise-form at most 3 times, submit-form at most 3 times per page/form. The first failure is not a stop. On the third form failure this same tool call pauses for the Studio manual-takeover card; wait until it returns resumedAfterManualTakeover and continue from its snapshot. The same click/choose/fill may fail only once. Never loop snapshot-click-snapshot or retry a failed selector. Do not record_stop+analyze a planned write with no successful response. Required number 0 is empty.",
    parameters: parameters({
