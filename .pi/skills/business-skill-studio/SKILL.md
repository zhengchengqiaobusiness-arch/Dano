...
1. **Record**
   ...
   - If the user says to run 查询/新增/修改 and fill every field except upload: snapshot, call `exercise-form` at most twice, then `submit-form` at most twice. Do not click or fill fields one by one after a failure. Do not skip optional or blank fields. Do not treat「请选择」or a required number `0` as already filled. A prompt like「请输入项目名称」next to a select is its own field. An empty avatar/plus well in a process rail is its own field; the node title is not a filled value. Upload/attachment widgets are the only skip. Locate a field by its own snapshot selector; never the first input in the dialog. Choosers must pick a real option, not a typed sample. `submit-form` is not success unless a form request is seen or the form closes.
   - Hard stop: the same `click` / `choose` / `fill` may fail only once. `exercise-form` ≤ 2, `submit-form` ≤ 2. If `ok` is false, `stopped` is true, or `followManualSteps` is true, do not retry that control and do not start a snapshot-click-snapshot or exercise-form idle loop.
   - If `snapshot.recordedManualSteps` or `session.manualStepsFile` (`manual-steps.md`) has steps, follow those recorded clicks/fills/choices exactly. Do not hunt for another selector. If the page is too complex to click, ask the user to switch to 手动录制 and operate the page; then use the written steps. 无法点击 / 无法复现 / 卡死 are forbidden.
