...
1. **Record**
   - Call `business_skill_record_start` with the real system URL. Put every operation explicitly requested by the user into `expectedOperations` (for example `查询+新增` becomes `["query", "create"]`). This caller-defined list is persisted unchanged and blocks partial review/export; later UI clicks and network evidence are recorded but never add unrequested requirements to it.
