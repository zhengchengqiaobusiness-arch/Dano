...
1. **Record**
   - Call `business_skill_record_start` with the real system URL. Put every operation explicitly requested by the user into `expectedOperations` (for example `查询+新增` becomes `["query", "create"]`). This list is persisted with the recording and blocks partial review/export.
   - Let the user operate the headed browser, or use `business_browser_control` after a `snapshot`.
   - Snapshot returns `formFields` / `todoFields` / `controls` plus `operationInventory` and enabled `availableOperations` across the main page and visible iframes. This is the source of truth for the page × operation checklist. When the user asks for every operation, exercise every applicable inventory item, not only the menu entry. Actual business clicks and their correlated requests are added to the recording completion contract automatically, including unfamiliar button text whose operation is established by the real HTTP method/path.
   - If the user says to run 查询/新增/修改 and fill every field except upload: snapshot, then call `exercise-form` once as the authoritative whole-form action. That single call must inspect and fill every currently visible eligible field in one pass; it must not intentionally split the initial form into one-field tool calls. Only when its returned `todoFields` is non-empty may the second or third call repair those remaining fields, newly revealed `新增一行` rows, tree/cascader choices, or format constraints. Then call `submit-form` at most 3 times. The first `exercise-form`/`submit-form` `ok: false` is not a stop. Do not click or fill fields one by one after a failure. Do not skip optional or blank fields, table rows revealed by `新增一行`/`添加明细`, or a second control that shares a label (`部门名称-2`). Do not treat「请选择」or a required number `0` as already filled. A prompt like「请输入项目名称」next to a select is its own field. An empty avatar/plus well in a process rail is its own field; the node title is not a filled value. Upload/attachment widgets are the only skip. Locate a field by its own snapshot selector; never the first input in the dialog. Choosers must pick a real option or tree node, not a typed sample. `submit-form` is not success unless a form request is seen or the form closes.
   - Hard stop: the same `click` / `choose` / `fill` may fail only once. `exercise-form` ≤ 3, `submit-form` ≤ 3. On the third failed form action, the same tool call pauses and Studio switches to manual mode with a takeover card. Do not issue another tool call, stop, analyze, or start a fallback path. Wait until the person finishes the current page and clicks「我已完成，继续自动执行」; then continue from the returned `resumedAfterManualTakeover` snapshot. The attempt budget is scoped to the current URL/form; a new page gets its own three attempts. 无法点击 / 无法复现 / 卡死 loops are forbidden.
