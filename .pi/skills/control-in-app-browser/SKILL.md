...
3. If the user asked to fill every field (上传/附件除外), call `exercise-form` at most twice, then `submit-form` at most twice. The first `exercise-form`/`submit-form` `ok: false` is not a stop: call the same action a second time so leftover fields, `新增一行`/`添加明细` rows, a second shared-label control (`部门名称-2`), and format repairs can finish. Do not click, fill, or choose individual fields after a failure. Do not skip optional or blank fields. The same `click` / `choose` / `fill` may fail only once. Stop that path only when the second call returns `followManualSteps`, or `stopped` is true. Locate must be unique: use the snapshot selector for that field, never the first input in the dialog. Empty assignee wells in a process rail (avatar/plus, not only a `+` button) are fields; the node title is not a filled value. Choosers open by clicking that field's shell once; pick a visible option or dialog row. Do not type a sample into a select filter. `submit-form.ok` is false unless a form request is seen or the form closes.
4. Follow `recordedManualSteps` or `manual-steps.md` only after `followManualSteps` or `stopped` is true. Do not switch to those steps after the first `exercise-form` failure. Do not hunt for another control. If the page cannot be clicked after the second failure, ask the user to switch to 手动录制. 无法点击 / 无法复现 / 卡死 loops are forbidden: no snapshot-click-snapshot, no third `exercise-form`, no retry of a failed selector.
