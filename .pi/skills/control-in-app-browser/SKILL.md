...
3. Read `.pi/skills/operate-form-controls/SKILL.md` and follow it. If the user asked to fill every field (上传/附件除外), snapshot then operate each `todoFields` item with `fill` / `choose` / `click`. `exercise-form` is only a mechanical helper; if it fails, finish leftover fields with primitives. Then `submit-form`. Locate with the snapshot selector. Do not type a sample into a select filter. `submit-form.ok` is false unless a form request is seen or the form closes.
