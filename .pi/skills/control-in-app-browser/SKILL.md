...
3. If the user asked to fill every field (上传/附件除外), call `exercise-form` on that scope. Treat `todoFields` as kit-agnostic: sibling inputs in the same group (including a prompt like「请输入项目名称」next to a select), nearby labels, readonly choosers, empty + slots, radios, shadow-root fields, same-origin iframe fields, and controls on other tabs. Locate a field by its own group, never the first input in the dialog. Choosers must pick a visible option or dialog row; do not type a sample into a select filter. Do not click long CSS paths. A required number showing `0` is still empty. Range dates use `rangeIndex`. Upload/attachment is the only skip. If `todoCount > 0` after one pass, call `exercise-form` once more. Do not submit while `todoCount > 0` or a required number is `0`. Prefer `submit-form` after the form is ready; it is not success unless a form request is seen or the form closes. If it returns errors, repair and submit again. Never click the page title or a heading to close a date picker or dropdown, never press Escape, and do not screenshot-loop.
