...
3. If the user asked to fill every field (上传/附件除外), call `exercise-form` at most twice, then `submit-form` at most twice. Do not click, fill, or choose individual fields after a failure. The same `click` / `choose` / `fill` may fail only once. If `ok` is false, `stopped` is true, or `followManualSteps` is true, stop that path. Locate must be unique: use the snapshot selector for that field, never the first input in the dialog. Empty assignee wells in a process rail (avatar/plus, not only a `+` button) are fields; the node title is not a filled value. Choosers open by clicking that field's shell once; pick a visible option or dialog row. Do not type a sample into a select filter. `submit-form.ok` is false unless a form request is seen or the form closes.
4. If `recordedManualSteps` or `manual-steps.md` exists, follow those steps exactly. Do not hunt for another control. If the page cannot be clicked, ask the user to switch to 手动录制. 无法点击 / 无法复现 / 卡死 loops are forbidden: no snapshot-click-snapshot, no extra `exercise-form`, no retry of a failed selector.
5. `recentUserActions` and `filled: true` are already observed. Do not replay those values unless the user asked to change them. Empty fields still must be filled when the user required complete coverage.
