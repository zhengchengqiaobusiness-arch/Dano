...
3. If the user asked to fill every field (上传/附件除外), call `exercise-form` on that scope. Table columns such as 产品名称/数量 belong in `todoFields`; do not click long CSS paths. If `todoCount > 0` after one pass, call `exercise-form` once more. Do not submit while `todoCount > 0`. Never click the page title or a heading such as 采购订单 to close a date picker or dropdown, never press Escape, and do not screenshot-loop; `exercise-form` / `choose` close leftover pickers.
