...
3. Read `.pi/skills/operate-form-controls/SKILL.md` and follow it. If the user asked to fill every field (上传/附件除外), the first `exercise-form` call is the authoritative whole-form action: it must fill every currently visible eligible field in one pass. Direct one-field `fill` / `choose` / `click` is only for repairing returned `todoFields` after that pass. Then `submit-form`. Locate with the snapshot selector. Do not type a sample into a select filter. `submit-form.ok` is false unless a form request is seen or the form closes.
