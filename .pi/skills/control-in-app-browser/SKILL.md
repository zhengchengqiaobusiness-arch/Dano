...
2. Treat `formFields` / `todoFields` as the field checklist for the **active scope** (`page` or `dialog`). Placeholder text such as「请选择」is empty, not filled.
3. If the user asked to fill every field (上传/附件除外), call `exercise-form` on that scope. Table columns such as 产品名称/数量 belong in `todoFields`; do not click long CSS paths. A required number showing `0` is still empty. If `todoCount > 0` after one pass, call `exercise-form` once more. Do not submit while `todoCount > 0` or a required number is `0`. Prefer `submit-form` after the form is ready; if it returns errors, repair and submit again. Never click the page title or a heading such as 采购订单 to close a date picker or dropdown, never press Escape, and do not screenshot-loop; `exercise-form` / `choose` close leftover pickers without clicking today's date.
