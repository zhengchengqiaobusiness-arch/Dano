...
3. If the user asked to fill every field (上传/附件除外), call `exercise-form` once as the authoritative whole-form action: its first call must inspect and fill all currently visible eligible fields in one pass. Do not decompose the initial form into one-field tool calls. A second or third `exercise-form` is allowed only for returned `todoFields`, newly revealed `新增一行`/`添加明细` rows, a second shared-label control (`部门名称-2`), tree/cascader nodes, or format repairs. Then call `submit-form` at most 3 times. The first `exercise-form`/`submit-form` `ok: false` is not a stop. Do not click, fill, or choose individual fields after a failure. Do not skip optional or blank fields. The same `click` / `choose` / `fill` may fail only once. Locate must be unique: use the snapshot selector for that field, never the first input in the dialog. Empty assignee wells in a process rail (avatar/plus, not only a `+` button) are fields; the node title is not a filled value. Choosers open by clicking that field's shell once; pick a visible option, tree node, or dialog row. Do not type a sample into a select filter. `submit-form.ok` is false unless a form request is seen or the form closes.
4. On the third failed `exercise-form`/`submit-form`, the same tool request pauses automatically and the Studio shows a manual-takeover card. Do not stop the recording, analyze, start another path, or issue another browser command. Wait for the person to complete the current page and click「我已完成，继续自动执行」; the pending tool call then returns `resumedAfterManualTakeover: true` with a fresh snapshot. Continue from that snapshot. The attempt budget is per page/form scope, so navigating to another page starts a fresh budget. Do not `record_stop` + analyze a planned 新增/修改 with no successful write response. 无法点击 / 无法复现 / 卡死 loops are forbidden: no snapshot-click-snapshot, no fourth `exercise-form`, no retry of a failed selector.
...
5. `recentUserActions` and `filled: true` are already observed. Do not replay those values unless the user asked to change them. Empty fields still must be filled when the user required complete coverage. Manual clicks or typing in automatic mode must record the same field labels, final values and form inventory as `exercise-form`. A bare click is not enough. When the user says 手动录制完毕, stop this session and analyze it; do not start a new recording because Chinese labels do not match pinyin request keys.
6. Prefer snapshot selectors: `placeholder=`, `label=`, `role=`, `text=`. Never use generated `#el-id-*` selectors.
7. Dropdowns: `choose` with the field selector and the visible option text in one call. Dates: `fill` or `choose` with `YYYY-MM-DD`. Never click a bare day number like `text=2` on the whole page, and never click the modal mask.
8. If a dialog is open, only operate inside it. Do not click background buttons such as 新增 again, and do not press Escape.
