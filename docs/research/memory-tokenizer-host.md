...
The extension's model-aware asynchronous callback is on independent repository
commit `3ccad8f`, not the published `0.1.0` dependency currently installed by Dano.
Publication, exact dependency integration and startup/shutdown ownership remain
required before enabling it in the application.
