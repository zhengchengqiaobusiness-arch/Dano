...
10 个任务全部执行完毕，后端 76 passed / 5 skipped（Pg 契约测试无 `DANO_PG_DSN`），
新增文件 `ruff check` 全绿，`npm run build` 通过。与计划的偏差如下：

1. **测试基准从 segno 换成 qrcode 包**。segno 默认给短文本选 Micro QR，且它的掩码
   选择与规范"最低罚分"不一致（同一份数据，segno 与 qrcode 的矩阵相差 718 个模块）。
   掩码编号写在格式信息里，任选一个都能被扫描器解出，所以"与某个库矩阵全等"不是
   合适的判据。最终判据改为两条：固定同一掩码时逐模块一致（验证码字/纠错/铺设/
   格式信息），以及断言确实选中了罚分最低的掩码。另加一条 OpenCV 解码器
   round-trip 测试（opencv 不入 dev 依赖，装了才跑）。
2. **格式信息的行列一度写反**，导致掩码选择连带偏移。参考实现用的是 `(col, row)`
   约定，本实现是 `matrix[row][col]`，已修正。
3. **challenge 存原始用户名而非 casefold 后的值**。租户查找大小写敏感，存 casefold
   值会让大小写混合的用户名在第二步查不到租户。限流键仍用 casefold。
4. **去掉 `totp_activated_at` 列**（计划中已说明的 YAGNI 偏差）。
5. `get_settings` 原先只在 app.py 的函数内局部导入，为路由改造提升到模块级。

### 一个值得记下的行为

同一个 TOTP 码不能既用于激活绑定又用于随后的登录——防重放游标会拒掉第二次。
真实使用中用户会等下一个码，但自动化测试必须用相邻时间步的码来绕开，
`tests/test_auth_routes.py::_bind_totp` 里有说明。
